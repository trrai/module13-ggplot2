...
ggplot(data = diamonds.samples) +
  geom_point(aes(x = carat, y = cut, size = price, color = price))
...
ggplot(data = diamonds.samples) +
  geom_smooth(aes(x = carat, y = price, color = carat))
